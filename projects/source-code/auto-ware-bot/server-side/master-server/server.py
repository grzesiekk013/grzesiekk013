        ...
        async for msg in websocket:
            ...
            if msg.startswith(CommunicationPrefixes.ANY_CMD.value):
                ...
                if current_time - last_any_cmd_time < rate_limit_sec:
                    _msg = f'{CommunicationPrefixes.ANY_CMD.value}{len(CARTON_ROBOT.COMMANDS)}{CommunicationPrefixes.ANY_CMD.value[::-1]}'
                    await websocket.send(_msg)
                    if DEBUG_PRINT: print(f'[WySŁANO] {_msg}')
                elif once == False:
                ...
                if cmd == "1":
                    ...
                    print(f"SKRĘT LEWO {val}")
                elif cmd == "3":
                    print(f"SKRĘT PRAWO {val}")
                print(f"L.KOM: {len(CARTON_ROBOT.COMMANDS)}")
                ...
                once = False
            elif msg.startswith(CommunicationPrefixes.NEW_TARGET.value):
                msg = msg.replace(CommunicationPrefixes.NEW_TARGET.value, "")
                msg = msg.replace(CommunicationPrefixes.NEW_TARGET.value[::-1], "")
                try:
                    msg_ = msg.split(';')
                    x0 = int(msg_[0])
                    x1 = int(msg_[1])
                    CARTON_ROBOT.TARGET_X = x0
                    CARTON_ROBOT.TARGET_Y = x1
                    await websocket.send(
                        f'{CommunicationPrefixes.NEW_TARGET.value}ok{CommunicationPrefixes.NEW_TARGET.value[::-1]}')
                except Exception as ex:
                    await websocket.send(
                        f'{CommunicationPrefixes.NEW_TARGET.value}no_ok{CommunicationPrefixes.NEW_TARGET.value[::-1]}')
                    print(f"{ex}")
